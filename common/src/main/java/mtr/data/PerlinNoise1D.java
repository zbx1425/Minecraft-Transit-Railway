...
public class PerlinNoise1D {
    ...
    public PerlinNoise1D(double baseFreq, int octaves, double stdDev) {
        this.stdDev = stdDev;
    ...
    public double getAt(double x) {
        // Amend x by cutoffFreq
        double x2 = x * baseFreq;
        return fbm((float)x2, octaves) * stdDev;
    }
    ...
    private static float fbm(float x, int octave) {
        var f = 0.0f;
        var w = 0.5f;
