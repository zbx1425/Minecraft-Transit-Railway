			...
			try {
				jsonConfig.get(CUSTOM_TRAINS_KEY).getAsJsonObject().entrySet().forEach(entry -> {
					try {
						...
						final String bveSoundBaseId = getOrDefault(jsonObject, CUSTOM_TRAINS_BVE_SOUND_BASE_ID, null, JsonElement::getAsString);
						...
						if (!baseTrainProperties.baseTrainType.isEmpty()) {
							if (bveSoundBaseId != null) {
								TrainClientRegistry.register(trainId, baseTrainType, baseTrainProperties.model, textureId, name, color, gangwayConnectionId, trainBarrierId, riderOffset, baseTrainProperties.bogiePosition, baseTrainProperties.isJacobsBogie, bveSoundBaseId, null);
							} else {
